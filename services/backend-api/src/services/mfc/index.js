...
function getNude (model_name, debug, callback){
  ...
  var child = spawn('bash', [filename, mkvPath, mfcUrl, model_name, filePath, debug])
    // Do async job
...
var Nude = {
  get: function(model_name, debug, callback) {
    //var video = getVideo(model_name)
    getNude(model_name, debug, function(data){
      callback(null, data)
    });
...
export const nude = (model_name, debug) => nudeFunct(model_name, debug)
